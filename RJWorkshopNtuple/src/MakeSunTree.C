...
int main(int argc, char* argv[]) {
  ...
  if ( argc < 2 ){
    ...
    cout << "  Example:      ./MakeSunTree.x -ifile=input.root -ofile=output.root"  << endl;
    cout << "  Example:      ./MakeSunTree.x -ilist=input.list -ofile=output.root"  << endl;
    cout << "  Example:      ./MakeSunTree.x -ifold=folder_path -ofile=output.root" << endl;
  ...
  for(int i = 0; i < Nfile; i++){
    ...
    TChain* chain = new TChain("FiducialEventsTree");
    chain->Add(filenames[i].c_str());
    cout << "   Running tree FiducialEventsTree " << chain->GetEntries() << endl;
    SunNtuple* ntuple = new SunNtuple(chain);
